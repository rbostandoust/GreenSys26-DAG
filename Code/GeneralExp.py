...
def main_parallel(instance_num_start, instance_num_end, version, start_date = pd.to_datetime("2024-01-01").date(), num_instances_per_day = 1):
    ...
    # candidate_makespan_slack_coeff = [1, 1.5, 2]
    log_file_path = f"../Logs/GeneralExpv2/{experiment_type}/{num_jobs}J_{num_machines}S_{num_operations_per_job}O_MeanOp={mean_duration_per_op_in_epoch}_v{version}.csv"
...
def main(start_date = pd.to_datetime("2024-01-01").date(), num_instances_per_day = 1):
    ...
    log_file_path = f"../Logs/GeneralExpv2/{experiment_type}/{num_jobs}J_{num_machines}S_{num_operations_per_job}O_MeanOp={mean_duration_per_op_in_epoch}.csv"
    directory = os.path.dirname(log_file_path)
    os.makedirs(directory, exist_ok=True)
    start_time = datetime.now()
    selected_date = start_date
